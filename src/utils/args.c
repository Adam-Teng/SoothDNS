...
static bool option_append_value(Option *opt, OptionValue value) {
  if (opt->count + 1 > opt->capacity) {
    ...
    OptionValue *new_array =
        realloc(opt->values, sizeof(OptionValue) * new_capacity);
    if (!new_array) {
      return false;
    }
...
static void ap_register_option(ArgParser *parser, const char *name,
                               Option *opt) {
  if (!opt) {
    ap_set_memory_error_flag(parser);
    return;
...
static void ap_parse_stream(ArgParser *parser, ArgStream *stream) {
  ...
  while (argstream_has_next(stream)) {
    ...
    else if (is_first_arg && parser->enable_help_command &&
             strcmp(arg, "help") == 0) {
...
// This function parses the application's command line arguments. It assumes
// that [argc] and [argv] are the arguments passed to main(), i.e. that the
// first element in [argv] is the program's name and should be ignored when
// parsing the actual *arguments*. In some situations [argv] can be empty, i.e.
// [argc == 0], which can lead to security vulnerabilities if not explicitly
// handled.
...
/*
 * Public function that init the global parameters of dns-relay
 */
parameter_t *parameter_init() {
  parameter_t *ret = (parameter_t *)malloc(sizeof(parameter_t));
  ret->server_addr = 0;
  ret->max_query = 0;
  ret->max_udp_req = 0;
  ret->client_port = 0;
...
char *help() {
  char *s =
      ...
      "  or:  ./soothDNS --help Display helper info.\n"
      "  or:  ./soothDNS -v     Display version info.\n"
      "\n"
      "Arguments:\n"
      "--server | -s <str>      Set remote DNS server address to <str>.\n"
